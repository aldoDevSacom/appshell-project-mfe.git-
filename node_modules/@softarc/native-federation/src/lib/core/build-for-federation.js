...
function logDuration(start, msg) {
    var [seconds, milliseconds] = process.hrtime(start);
    logger_1.logger.debug(`${seconds}s:${milliseconds.toFixed(3)}ms - ${msg}`);
}
...
function bundleSeparate(separateBrowser, externals, config, fedOptions, platform) {
    return tslib_1.__awaiter(this, void 0, void 0, function* () {
        const bundlePromises = Object.entries(separateBrowser).map((_a) => tslib_1.__awaiter(this, [_a], void 0, function* ([key, shared]) {
            const packageName = inferPackageFromSecondary(key);
            const filteredExternals = externals.filter((e) => !e.startsWith(packageName));
            return (0, bundle_shared_1.bundleShared)({ [key]: shared }, config, fedOptions, filteredExternals, platform);
        }));
        const buildResults = yield Promise.all(bundlePromises);
